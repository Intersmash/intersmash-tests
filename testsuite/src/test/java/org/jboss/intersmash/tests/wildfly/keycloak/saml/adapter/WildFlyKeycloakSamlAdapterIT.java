...
public class WildFlyKeycloakSamlAdapterIT {
	/** HTML element ID containing the authenticated username. */
	private static final String HTML_ID_WITH_USERNAME = "username";

	/** Route URL for the WildFly/JBoss EAP application. */
	@ServiceUrl(WildflyWithKeycloakSamlAdapterApplication.class)
	...
	public void testSamlClientCreation() {
		Pod wildflyPod = OpenShifts.master().getLabeledPods("name", WildflyWithKeycloakSamlAdapterApplication.APP_NAME).get(0);
	...
	public void testAccessOk() throws IOException {
		...
		try (final WebClient webClient = new WebClient()) {
			...
			LoginUtil.assertIsExpectedRealm(keycloakLoginPage,
					BasicKeycloakOperatorDynamicClientSamlApplication.REALM_NAME);

			HtmlPage securedPage = (HtmlPage) LoginUtil.makeLogin(keycloakLoginPage,
			...
			LoginUtil.assertIsSecuredPage(securedPage);
			// then we check the JSP page received the expected security info
			LoginUtil.assertIsUserPrincipal(securedPage, HTML_ID_WITH_USERNAME,
					BasicKeycloakOperatorDynamicClientSamlApplication.USER_NAME);
		}
	}
	...
	public void testForbidden() throws IOException {
		...
		try (final WebClient webClient = new WebClient()) {
			...
			LoginUtil.assertIsForbidden(securedPage);
		}
	}
}
