...
public class WildFlyBootableJarKeycloakSamlAdapterEjbHelmIT {
	...
	private String wildflySecuredPage;

	/** The URL of the unsecured page that doesn't require authentication. */
	private String wildflyUnSecuredPage;
	...
	public void beforeEach() {
		wildflySecuredPage = wildflyApplicationRouteUrl.replace("http://", "https://") + "/secured";
		wildflyUnSecuredPage = wildflyApplicationRouteUrl.replace("http://", "https://") + "/unsecured";
		log.debug("wildfly secured url: {}", wildflySecuredPage);
		log.debug("wildfly unsecured url: {}", wildflyUnSecuredPage);
	}
	...
	public void testWildflyApplicationIsRunning() {
		log.info("https base url: {}", wildflyUnSecuredPage);
		HttpsUtils.doesUrlReturnOK(wildflyUnSecuredPage).timeout(5000).waitFor();
	...
	public void testAccessOk() throws IOException {
		...
		try (final WebClient webClient = new WebClient()) {
			...
			HtmlPage loginPage = webClient.getPage(wildflySecuredPage);
