...
/**
 * WildFly/JBoss EAP + Keycloak/RHBK interoperability tests.
 *
 * WildFly/JBoss EAP application secured by Keycloak/RHBK.
 * The Keycloak client is provided to the WildFly/JBoss EAP application by the elytron-oidc-client layer.
 *
...
@Intersmash({
		...
		@Service(BasicKeycloakOperatorOidcHttpsApplication.class),
...
public class WildflyBootableJarWithElytronOidcClientIT {
	...
	public void beforeEach() {
		...
		log.debug("WildFly secured url: {}", wildFlyBootableJarSecuredPage);
		log.debug("WildFly unsecured url: {}", wildFlyBootableJarUnSecuredPage);
	}
	...
	public void testSuccess() throws IOException {
		...
		Page securedPage = requestSecuredPageAndLogin(
				BasicKeycloakOperatorOidcHttpsApplication.USER_NAME_WITH_CORRECT_ROLE,
				BasicKeycloakOperatorOidcHttpsApplication.USER_PASSWORD_WITH_CORRECT_ROLE);
	...
	public void testForbidden() throws IOException {
		assumeTrue(wildflyApplicationIsRunning);
		Page securedPage = requestSecuredPageAndLogin(
				BasicKeycloakOperatorOidcHttpsApplication.USER_NAME_WITH_WRONG_ROLE,
				BasicKeycloakOperatorOidcHttpsApplication.USER_PASSWORD_WITH_WRONG_ROLE);
	...
	public void testUnauthorized() throws IOException {
		assumeTrue(wildflyApplicationIsRunning);
		Page page = requestSecuredPageAndLogin(
				BasicKeycloakOperatorOidcHttpsApplication.USER_NAME_WITH_CORRECT_ROLE,
				"wrong_password");
	...
	private Page requestSecuredPageAndLogin(String login, String password) throws IOException {
		try (final WebClient webClient = new WebClient()) {
			...
			return LoginUtil.makeLogin(loginPage, login, password);
		}
	}
	...
	private static String contentOf(Page page) {
		return LoginUtil.contentOf(page);
	}
}
