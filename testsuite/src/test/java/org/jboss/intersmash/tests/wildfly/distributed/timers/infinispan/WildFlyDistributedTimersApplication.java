...
public class WildFlyDistributedTimersApplication
	...
	private static final String TRUSTSTORE_FOLDER = "/etc/secrets";
	...
	public WildFlyDistributedTimersApplication() throws IOException {
		...
		secrets.add(new SecretBuilder("infinispan-client-secret")
				.addData(TRUSTSTORE_FILE_NAME, CommandLineBasedKeystoreGenerator.getTruststore())
		...
		environmentVariables.add(new EnvVarBuilder()
				.withName("JDG_HOST")
				.withValue("$(INFINISPAN_SERVICE_HOST)")
				.build());
		environmentVariables.add(new EnvVarBuilder()
				.withName("JDG_PORT")
				.withValue("$(INFINISPAN_SERVICE_PORT)")
				.build());
		environmentVariables.add(new EnvVarBuilder()
				.withName("TRUST_STORE_PASSWORD")
				.withValue(truststorePassword)
				.build());
		// By default, secrets are mounted in /etc/secrets, hence the trustore would end up in the container at /etc/secrets/truststore.jks
		environmentVariables.add(new EnvVarBuilder()
				.withName("TRUST_STORE_PATH")
				.withValue(String.format("%s/%s", TRUSTSTORE_FOLDER, TRUSTSTORE_FILE_NAME))
				.build());
		environmentVariables.add(new EnvVarBuilder()
				.withName("TRUST_STORE_TYPE")
				.withValue("PKCS12")
				.build());
		...
		environmentVariables.add(new EnvVarBuilder()
				...
				.withValue(IntersmashConfig.scriptDebug() != null ? IntersmashConfig.scriptDebug() : "false").build());
	}
