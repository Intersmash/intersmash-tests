...
public class OpenShiftBinaryClient {
	...
	public static OpenShiftBinaryClient getAdminInstance() {
		if (INSTANCE == null) {
			try {
				INSTANCE = new OpenShiftBinaryClient();
				// call oc login to create ~/.kube/config
	...
	public void executeCommand(String error, String... args) {
		String[] ocArgs = ArrayUtils.addAll(new String[] { ocBinaryPath, "--kubeconfig=" + CONFIG_FILE.getAbsolutePath() },
				args);
		try {
	...
	public String executeCommandWithReturn(final String error, String... args) {
		String[] ocArgs = ArrayUtils
				.addAll(new String[] { ocBinaryPath, "--kubeconfig=" + CONFIG_FILE.getAbsolutePath() }, args);
		try {
	 ...
	 * @param args command arguments
	 */
	private static Process executeLocalCommand(String error, boolean wait, final boolean needOutput, final boolean needError,
		...
		log.info("Executing local command: {}", String.join(" ", args));
	...
	private static void login() throws IOException, InterruptedException {
		...
		loginStatic(masterIp, OpenShiftConfig.masterUsername(), OpenShiftConfig.masterPassword(),
				OpenShiftConfig.masterToken());
	}

	public void login(String masterUrl, String masterUserName, String masterPassword, String masterToken)
	...
	public static void loginAdmin() throws IOException, InterruptedException {
		loginStatic(OpenShiftConfig.url(), OpenShiftConfig.adminUsername(), OpenShiftConfig.adminPassword(),
				OpenShiftConfig.adminToken());
	}

	private static void loginStatic(String masterUrl, String masterUserName, String masterPassword, String masterToken)
			throws IOException, InterruptedException {
